...
def download(url, dest):
  ...
  if os.path.exists(dest):
    logger.debug(f"{dest} already exists. Skipping download from {url}")
    return
...
def ts07(year, doy, url=url_ts07):
  ...
  def extract_year_doy_tgz(year, doy):
    ...
    logger.debug(f"Looking for {year_doy_tgz} in {all_tgz}")
    if os.path.exists(year_doy_tgz):
      logger.debug(f"{year_doy_tgz} already extracted from {all_tgz}. Skipping extraction.")
    else:
      with tarfile.open(all_tgz, 'r:*') as tar:
        for member in tar.getmembers():
          if member.name == year_doy_file:
            logger.debug(f"Extracting {year_doy_tgz} to {all_tgz_dir}")
  ...
  def extract_year_doy_coeffs(year, doy):
    ...
    if os.path.exists(path):
      logger.debug(f"{path} already exists. Skipping extraction from {year_doy_tgz}")
    else:
      with tarfile.open(year_doy_tgz, 'r:*') as tar:
        logger.debug(f"Extracting {year_doy_tgz} to {path}")
        tar.extractall(path=path)
        # Move the extracted directory one directory up
        extracted_dir = os.path.join(all_tgz_dir, f"{year}_{doy:03d}")
        if os.path.exists(extracted_dir):
            logger.debug(f"{extracted_dir} already exists. Skipping move.")
        else:
            logger.debug(f"Moving {path} to {extracted_dir}")
